...
Next, you have to make the `test()` and `expect()` functions be available _globally_ for the `greet.test.ts` file. To do that, use the `--loader` option on the Node.js CLI.
...
Finally, to run the tests, use the `--loader` option of the Node.js CLI to reference the `setup.ts` as a global file to include:
