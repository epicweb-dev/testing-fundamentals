...
I will start by making the `test()` function in `setup.js` asynchronous.

<CodeFile file="setup.js" range="11-12" highlight="11" />

This will allow me to await the `callback()` if it happens to be `async` too.

<CodeFile file="setup.js" range="11-19" highlight="13" />
